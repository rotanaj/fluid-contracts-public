...
contract VaultsBaseTest is VaultFactoryBaseTest, PoolT1BaseTest {
    ...
    function setUp() public virtual override(PoolT1BaseTest, VaultFactoryBaseTest) {
        ...
        _vaultResolver = new FluidVaultResolver(address(vaultFactory), address(liquidityResolver_));
...

        {
            ...
            address vaultT1OperatePointer_ = _sstore2Deployer.deployCode(
                type(FluidVaultT1Operate_Not_For_Prod).creationCode
            );
        ...
        {
            // Vault T4
            (address vaultT4Pointer1_, address vaultT4Pointer2_) = _sstore2Deployer.deployCodeSplit(
                type(FluidVaultT4).creationCode
            );
    ...
    function _deployDexVaults(DexParams memory dex_) internal returns (DexVaultParams memory dexVault_) {
        bytes memory vaultT1CreationCode = abi.encodeCall(
            _vaultT1NotForProdDeploymentLogic.vaultT1,
            (dex_.token0, dex_.token1)
        );
        bytes memory vaultT2CreationCode = abi.encodeCall(
            _vaultT2DeploymentLogic.vaultT2,
            (address(dex_.dexCol), dex_.token1)
        );
        bytes memory vaultT3CreationCode = abi.encodeCall(
            _vaultT3DeploymentLogic.vaultT3,
            (dex_.token0, address(dex_.dexDebt))
        );
        bytes memory vaultT4CreationCode = abi.encodeCall(
            _vaultT4DeploymentLogic.vaultT4,
            (address(dex_.dexColDebt), address(dex_.dexColDebt))
        );

        vm.startPrank((admin));
        dexVault_.vaultT1 = FluidVaultT1_Not_For_Prod(
            payable(vaultFactory.deployVault(address(_vaultT1NotForProdDeploymentLogic), vaultT1CreationCode))
        );
        dexVault_.vaultT2 = FluidVaultT2(
            payable(vaultFactory.deployVault(address(_vaultT2DeploymentLogic), vaultT2CreationCode))
        );
        dexVault_.vaultT3 = FluidVaultT3(
            payable(vaultFactory.deployVault(address(_vaultT3DeploymentLogic), vaultT3CreationCode))
        );
        dexVault_.vaultT4 = FluidVaultT4(
            payable(vaultFactory.deployVault(address(_vaultT4DeploymentLogic), vaultT4CreationCode))
        );
        vm.stopPrank();
        ...
        vm.label(
            address(dexVault_.oracleT1),
            string(abi.encodePacked(_getDexVaultPoolName(dexVault_, VaultType.VaultT1), ":oracleT1"))
        );
        vm.label(
            address(dexVault_.oracleT2),
            string(abi.encodePacked(_getDexVaultPoolName(dexVault_, VaultType.VaultT2), ":oracleT2"))
        );
        vm.label(
            address(dexVault_.oracleT3),
            string(abi.encodePacked(_getDexVaultPoolName(dexVault_, VaultType.VaultT3), ":oracleT3"))
        );
        vm.label(
            address(dexVault_.oracleT4),
            string(abi.encodePacked(_getDexVaultPoolName(dexVault_, VaultType.VaultT4), ":oracleT4"))
        );
    }
    ...
    function _setUpDexVault(DexVaultParams memory dexVault_, uint256 priceInWei_) internal {
        ...
        _setUserAllowancesDefaultWithModeWithHighLimit(
            address(liquidity),
            address(admin),
            address(dexVault_.dex.token0),
            address(dexVault_.vaultT1),
            true
        );
        _setUserAllowancesDefaultWithModeWithHighLimit(
            address(liquidity),
            address(admin),
            address(dexVault_.dex.token1),
            address(dexVault_.vaultT1),
            true
        );
    ...
    function _getDexVaultPoolName(
        DexVaultParams memory dexVault_,
        VaultType vaultType_
    ) internal returns (string memory s_) {
        s_ = string(abi.encodePacked(dexVault_.vaultName, ":::"));
    ...
    function _calculatePrice(
        ...
        VaultType vaultType_,
        uint256 priceInWei_
    ) internal returns (uint256 priceIn27Decimals_) {
        if (vaultType_ == VaultType.VaultT1) {
    ...
    function _getDexTypeAddressForVault(
        DexVaultParams memory dexVault_,
        VaultType vaultType_
    ...
    function _getDexVaultState(
        DexVaultParams memory dexVault_,
        VaultType vaultType_,
        address user_,
        uint256 nftId_
    ) internal returns (VaultDexStateData memory) {
        ...
        FluidVaultPositionsResolver.UserPosition[] memory userPositions_ = _vaultPositionsResolver
            .getPositionsForNftIds(nftIds_);
        ...
        VaultDexStateData memory s_ = VaultDexStateData({
            ...
            totalDexSupplyShares: vaultType_ == VaultType.VaultT1 || vaultType_ == VaultType.VaultT3
                ? 0
                : getTotalSupplyShares(dex_),
            totalDexBorrowShares: vaultType_ == VaultType.VaultT1 || vaultType_ == VaultType.VaultT2
                ? 0
                : getTotalBorrowShares(dex_),
            userDexSupplyShares: vaultType_ == VaultType.VaultT1 || vaultType_ == VaultType.VaultT3
                ? 0
                : getUserSupplyShare(dex_, vault_),
            userDexBorrowShares: vaultType_ == VaultType.VaultT1 || vaultType_ == VaultType.VaultT2
                ? 0
                : getUserBorrowShare(dex_, vault_),
    ...
    function _testOperatePerfect(
        ...
        var_.assertMessage = string(
            abi.encodePacked(customMessage_, "::_testOperatePerfect::", _getDexVaultPoolName(dexVault_, vaultType_))
        );
        ...
        var_.ethValue = 0;
        if (
            dexVault_.dex.token0 == address(NATIVE_TOKEN_ADDRESS) ||
            dexVault_.dex.token1 == address(NATIVE_TOKEN_ADDRESS)
        ) {
...

        {
            ...
            if (vaultType_ == VaultType.VaultT1) {
                assertApproxEqAbs(
                    int256(postState_.liquidityToken0Balance) - int256(preState_.liquidityToken0Balance),
                    d_.supplyAmount,
                    0,
                    string(abi.encodePacked(var_.assertMessage, "liquidity balance token0 is not expected"))
                );
                assertApproxEqAbs(
                    int256(postState_.liquidityToken1Balance) - int256(preState_.liquidityToken1Balance),
                    -(d_.borrowAmount),
                    0,
                    string(abi.encodePacked(var_.assertMessage, "liquidity balance token1 is not expected"))
                );

                assertApproxEqAbs(
                    int256(preState_.userToken0Balance) - int256(postState_.userToken0Balance),
                    d_.supplyAmount,
                    0,
                    string(abi.encodePacked(var_.assertMessage, "user balance token0 is not expected"))
                );
                assertApproxEqAbs(
                    int256(preState_.userToken1Balance) - int256(postState_.userToken1Balance),
                    -(d_.borrowAmount),
                    0,
                    string(abi.encodePacked(var_.assertMessage, "user balance token1 is not expected"))
                );
            } else if (vaultType_ == VaultType.VaultT2) {
                assertApproxEqAbs(
                    int256(postState_.liquidityToken0Balance) - int256(preState_.liquidityToken0Balance),
                    d_.supplyToken0Amount,
                    0,
                    string(abi.encodePacked(var_.assertMessage, "liquidity balance token0 is not expected"))
                );
                assertApproxEqAbs(
                    int256(postState_.liquidityToken1Balance) - int256(preState_.liquidityToken1Balance),
                    -(d_.borrowAmount) + d_.supplyToken1Amount,
                    0,
                    string(abi.encodePacked(var_.assertMessage, "liquidity balance token1 is not expected"))
                );
                ...
                assertApproxEqAbs(
                    int256(postState_.totalDexSupplyShares) - int256(preState_.totalDexSupplyShares),
                    (d_.supplyAmount),
                    0,
                    string(abi.encodePacked(var_.assertMessage, "total dex supply shares is not expected"))
                );
                assertApproxEqAbs(
                    int256(postState_.totalDexBorrowShares) - int256(preState_.totalDexBorrowShares),
                    0,
                    0,
                    string(abi.encodePacked(var_.assertMessage, "total dex borrow shares is not expected"))
                );
                ...
                assertApproxEqAbs(
                    int256(postState_.userDexSupplyShares) - int256(preState_.userDexSupplyShares),
                    (d_.supplyAmount),
                    1,
                    string(abi.encodePacked(var_.assertMessage, "user dex supply shares is not expected"))
                );
                assertApproxEqAbs(
                    int256(postState_.userDexBorrowShares) - int256(preState_.userDexBorrowShares),
                    0,
                    0,
                    string(abi.encodePacked(var_.assertMessage, "user dex borrow shares is not expected"))
                );
            } else if (vaultType_ == VaultType.VaultT3) {
                assertApproxEqAbs(
                    int256(postState_.liquidityToken0Balance) - int256(preState_.liquidityToken0Balance),
                    d_.supplyAmount - (d_.borrowToken0Amount),
                    0,
                    string(abi.encodePacked(var_.assertMessage, "liquidity balance token0 is not expected"))
                );
                assertApproxEqAbs(
                    int256(postState_.liquidityToken1Balance) - int256(preState_.liquidityToken1Balance),
                    -(d_.borrowToken1Amount),
                    0,
                    string(abi.encodePacked(var_.assertMessage, "liquidity balance token1 is not expected"))
                );
                ...
                assertApproxEqAbs(
                    int256(postState_.liquidityToken0Balance) - int256(preState_.liquidityToken0Balance),
                    d_.supplyToken0Amount - (d_.borrowToken0Amount),
                    0,
                    string(abi.encodePacked(var_.assertMessage, "liquidity balance token0 is not expected"))
                );
                assertApproxEqAbs(
                    int256(postState_.liquidityToken1Balance) - int256(preState_.liquidityToken1Balance),
                    d_.supplyToken1Amount - (d_.borrowToken1Amount),
                    0,
                    string(abi.encodePacked(var_.assertMessage, "liquidity balance token1 is not expected"))
                );
            ...
            assertApproxEqAbs(
                int256(postState_.totalVaultSupply) - int256(preState_.totalVaultSupply),
                (d_.supplyAmount),
                1,
                string(abi.encodePacked(var_.assertMessage, "total vault supply is not expected"))
            );
            ...
            uint256 weiDelta = ((
                postState_.totalVaultBorrow > preState_.totalVaultBorrow
                    ? postState_.totalVaultBorrow
                    : preState_.totalVaultBorrow
            ) * (1e9)) /
                1e18 +
                8196 +
                (a < 0 ? uint256(-a) : uint256(a));

            assertApproxEqAbs(
                int256(postState_.totalVaultBorrow) - int256(preState_.totalVaultBorrow),
                (d_.borrowAmount),
                weiDelta,
                string(abi.encodePacked(var_.assertMessage, "total vault borrow is not expected"))
            );

            assertApproxEqAbs(
                int256(postState_.userVaultSupplyBalance) - int256(preState_.userVaultSupplyBalance),
                (d_.supplyAmount),
                1,
                string(abi.encodePacked(var_.assertMessage, "user vault supply balance is not expected"))
            );
            ...
            assertApproxEqAbs(
                int256(postState_.userVaultBorrowBalance) - int256(preState_.userVaultBorrowBalance),
                (d_.borrowAmount),
                weiDelta,
                string(abi.encodePacked(var_.assertMessage, "user vault borrow balance is not expected"))
            );
        }
    }
    ...
    function _testOperate(
        ...
        OperatePerfectVariables memory var_;
        var_.assertMessage = string(
            abi.encodePacked(customMessage_, "::_testOperate::", _getDexVaultPoolName(dexVault_, vaultType_))
        );
        ...
        assertApproxEqAbs(
            int256(postState_.liquidityToken0Balance) - int256(preState_.liquidityToken0Balance),
            params_.supplyToken0 - params_.borrowToken0,
            maxDetla_,
            string(abi.encodePacked(var_.assertMessage, "liquidity balance token0 is not expected"))
        );
        assertApproxEqAbs(
            int256(postState_.liquidityToken1Balance) - int256(preState_.liquidityToken1Balance),
            -params_.borrowToken1 + params_.supplyToken1,
            maxDetla_,
            string(abi.encodePacked(var_.assertMessage, "liquidity balance token1 is not expected"))
        );

        assertApproxEqAbs(
            int256(postState_.userToken0Balance) - int256(preState_.userToken0Balance),
            params_.borrowToken0 - params_.supplyToken0,
            0,
            string(abi.encodePacked(var_.assertMessage, "user balance token0 is not expected"))
        );
        assertApproxEqAbs(
            int256(postState_.userToken1Balance) - int256(preState_.userToken1Balance),
            params_.borrowToken1 - params_.supplyToken1,
            0,
            string(abi.encodePacked(var_.assertMessage, "user balance token1 is not expected"))
        );
        ...
        if (vaultType_ == VaultType.VaultT1) {
            assertApproxEqAbs(
                ...
                string(
                    abi.encodePacked(
                        var_.assertMessage,
                        "params_.supplyToken0 != d_.supplyAmount token0 is not expected"
                    )
                )
            );
            assertApproxEqAbs(
                ...
                string(
                    abi.encodePacked(
                        var_.assertMessage,
                        "params_.borrowToken1 != d_.borrowAmount token1 is not expected"
                    )
                )
            );
        } else if (vaultType_ == VaultType.VaultT2) {
    ...
    function _getDexWithdrawPerfectInOneTokenAmount(
        ...
        uint256 amt_;
        try
            dex_.withdrawPerfectInOneToken(
                shareAmount_,
    ...
    function _getDexDepositPerfect(
        ...
        uint256 amt0_;
        uint256 amt1_;
        try dex_.depositPerfect(shareAmount_, type(uint256).max, type(uint256).max, true) {
            // Handle successful execution
        } catch Error(string memory) {
            // Handle generic errors with a reason
        } catch (bytes memory lowLevelData_) {
    ...
    function _getDexPaybackPerfect(
        ...
        uint256 amt0_;
        uint256 amt1_;
        try dex_.paybackPerfect(shareAmount_, type(uint256).max, type(uint256).max, true) {
            // Handle successful execution
        } catch Error(string memory) {
            // Handle generic errors with a reason
        } catch (bytes memory lowLevelData_) {
    ...
    function _testLiquidate(
        ...
        l_.colateralAmount = l_.liquidationData.outAmt > l_.liquidationData.outAmtWithAbsorb
            ? l_.liquidationData.outAmt
            : l_.liquidationData.outAmtWithAbsorb;
        l_.debtAmount = l_.liquidationData.inAmt > l_.liquidationData.inAmtWithAbsorb
            ? l_.liquidationData.inAmt
            : l_.liquidationData.inAmtWithAbsorb;
        ...
        if (vaultType_ == VaultType.VaultT1) {
            ...
            vm.prank(user_);
            (d_.debtShares, d_.colShares, d_.token0Col, d_.token1Col) = dexVault_.vaultT2.liquidate{
                value: l_.debtAmount
            }(
                l_.debtAmount,
                0, // colPerUnitDebt
            ...
            if (params_.debtToken0Percentage > 0 && params_.debtToken1Percentage > 0) {
                ...
                if (vaultType_ == VaultType.VaultT3) {
                    vm.prank(user_);
                    (d_.debtShares, d_.token0Debt, d_.token1Debt, d_.colShares) = dexVault_.vaultT3.liquidatePerfect{
                        value: (l_.debtToken1Amt * l_.debtAmount) / 1e18
                    }(
                        (l_.debtAmount * params_.debtToken0Percentage) / 100,
                        l_.debtToken0Amt,
                        l_.debtToken1Amt,
                    ...
                    vm.prank(user_);
                    (d_.debtShares, d_.token0Debt, d_.token1Debt, d_.colShares, d_.token0Col, d_.token1Col) = dexVault_
                        .vaultT4
                        .liquidatePerfect{ value: (l_.debtToken1Amt * l_.debtAmount) / 1e18 }(
                        (l_.debtAmount * params_.debtToken0Percentage) / 100,
                        l_.debtToken0Amt,
                        l_.debtToken1Amt,
                        0, // colPerUnitDebt
                ...
                if (vaultType_ == VaultType.VaultT3) {
                    ...
                    vm.prank(user_);
                    (d_.debtShares, d_.colShares, d_.token0Col, d_.token1Col) = dexVault_.vaultT4.liquidate{
                        value: d_.token1Debt
                    }(
                        d_.token0Debt,
                        d_.token1Debt,
    ...
    function _convertOperatePerfectParamsToWei(
        DexVaultParams memory dexVault_,
        VaultType vaultType_,
        OperatePerfectParams memory params_
    ) internal returns (OperatePerfectParams memory updatedParams_) {
        if (vaultType_ == VaultType.VaultT1) {
    ...
    function _convertOperateParamsToWei(
        DexVaultParams memory dexVault_,
        VaultType vaultType_,
        OperateParams memory params_
    ) internal returns (OperateParams memory updatedParams_) {
        if (vaultType_ == VaultType.VaultT1) {
    ...
    function _simulateRebalance(
        ...
        vm.prank(user_);
        try
            FluidVaultT2(payable(vault_)).rebalance{ value: ethValue_ }(
    ...
    function _getRebalanceAmounts(
        ...
        (, bytes memory data_) = address(this).call(
            abi.encodeWithSelector(this._simulateRebalance.selector, vault_, params_, ethValue_, user_)
        );
    ...
    function _getRebalanceState(
        DexVaultParams memory dexVault_,
        VaultType vaultType_
    ) internal returns (RebalanceDataParams memory params_) {
        ...
        (FluidLiquidityResolver.UserSupplyData memory userSupplyDataToken0_, ) = resolver.getUserSupplyData(
            address(v_.vault),
            dexVault_.dex.token0
        );
        (FluidLiquidityResolver.UserBorrowData memory userBorrowDataToken1_, ) = resolver.getUserBorrowData(
            address(v_.vault),
            dexVault_.dex.token1
        );
        ...
        if ((vaultType_ == VaultType.VaultT2 || vaultType_ == VaultType.VaultT4)) {
            FluidDexT1 dex_ = FluidDexT1(payable(_getDexTypeAddressForVault(dexVault_, vaultType_)));
            v_.totalSupply =
                (getUserSupplyShare(dex_, v_.vault) * v_.liqSupplyExPrice) /
        ...
        if ((vaultType_ == VaultType.VaultT3 || vaultType_ == VaultType.VaultT4)) {
            FluidDexT1 dex_ = FluidDexT1(payable(_getDexTypeAddressForVault(dexVault_, vaultType_)));
            v_.totalBorrow =
                (getUserBorrowShare(dex_, v_.vault) * v_.liqBorrowExPrice) /
                LiquidityCalcs.EXCHANGE_PRICES_PRECISION;
        } else {
        ...
        v_.totalSupplyVault =
            (vaultVariables_.totalSupply * v_.vaultSupplyExPrice) /
            LiquidityCalcs.EXCHANGE_PRICES_PRECISION;
        v_.totalBorrowVault =
            (vaultVariables_.totalBorrow * v_.vaultBorrowExPrice) /
            LiquidityCalcs.EXCHANGE_PRICES_PRECISION;
    ...
    function _testRebalance(
        ...
        var_.assertMessage = string(
            abi.encodePacked(
                ...
                _getDexVaultPoolName(dexVault_, vaultType_),
                "::",
                params_.depositOrWithdraw == 1
                    ? "deposit"
                    : params_.depositOrWithdraw == 2
                        ? "withdraw"
                        : "none",
                "::",
                params_.borrowOrPayback == 1
                    ? "borrow"
                    : params_.borrowOrPayback == 2
                        ? "payback"
                        : "none",
                "::",
        ...
        RebalanceAmountData memory amts_ = _getRebalanceAmounts(
            dexVault_,
            vaultType_,
            var_.vault,
            var_.params,
...

        {
            var_.params = _getRebalanceState(dexVault_, vaultType_);
            (, bytes memory data_) = address(this).call(
                abi.encodeWithSelector(this._simulateRebalance.selector, var_.vault, var_.params, var_.ethValue, user_)
            );
            ...
            assertApproxEqAbs(
                ...
                0,
                0,
                string(abi.encodePacked(var_.assertMessage, "rebalance supply not zero"))
            );
            assertApproxEqAbs(
                d_.borrowAmount,
                0,
                0,
                string(abi.encodePacked(var_.assertMessage, "rebalance borrow not zero"))
            );
        }
    }
    ...
    function test_perfectSupplyBorrowPaybackWithdraw() public {
        ...
        for (uint256 i = 0; i < vaults_.length; i++) {
            for (uint256 j = 0; j < vaultTypes_.length; j++) {
                ...
                OperatePerfectData memory d_ = _testOperatePerfect(
                    vaults_[i],
                    vaultTypes_[j],
                    _convertOperatePerfectParamsToWei(vaults_[i], vaultTypes_[j], supplyParams_),
                    alice,
                    0,
                    "test_perfectSupplyBorrowPaybackWithdraw(supply)"
                );
                ...
                OperatePerfectData memory db_ = _testOperatePerfect(
                    vaults_[i],
                    vaultTypes_[j],
                    _convertOperatePerfectParamsToWei(vaults_[i], vaultTypes_[j], borrowParams_),
                    alice,
                    d_.nftId,
                    "test_perfectSupplyBorrowPaybackWithdraw(borrow)"
                );
                ...
                OperatePerfectData memory dp_ = _testOperatePerfect(
                    vaults_[i],
                    vaultTypes_[j],
                    paybackParams_,
                    alice,
                    d_.nftId,
                    "test_perfectSupplyBorrowPaybackWithdraw(payback)"
                );
                ...
                OperatePerfectData memory dw_ = _testOperatePerfect(
                    vaults_[i],
                    vaultTypes_[j],
                    withdrawParams_,
                    alice,
                    d_.nftId,
                    "test_perfectSupplyBorrowPaybackWithdraw(withdraw)"
                );
            }
        }
    }
...
contract VaultsOperateTest is VaultsBaseTest {
    function test_supplyBorrowPaybackWithdraw() public {
        ...
        for (uint256 i = 0; i < vaults_.length; i++) {
            for (uint256 j = 0; j < vaultTypes_.length; j++) {
                ...
                OperateData memory d_ = _testOperate(
                    vaults_[i],
                    vaultTypes_[j],
                    _convertOperateParamsToWei(vaults_[i], vaultTypes_[j], supplyParams_),
                    alice,
                    0,
                    "test_supplyBorrowPaybackWithdraw(supply)"
                );
                _testOperate(
                    vaults_[i],
                    vaultTypes_[j],
                    _convertOperateParamsToWei(vaults_[i], vaultTypes_[j], borrowParams_),
                    alice,
                    d_.nftId,
                    "test_supplyBorrowPaybackWithdraw(borrow)"
                );
                _testOperate(
                    vaults_[i],
                    vaultTypes_[j],
                    ...
                    alice,
                    d_.nftId,
                    "test_supplyBorrowPaybackWithdraw(payback)"
                );
                _testOperate(
                    vaults_[i],
                    vaultTypes_[j],
                    ...
                    alice,
                    d_.nftId,
                    "test_supplyBorrowPaybackWithdraw(withdraw)"
                );
            }
        }
    }
        ...
        for (uint256 i = 0; i < vaults_.length; i++) {
            for (uint256 j = 0; j < vaultTypes_.length; j++) {
                ...
                OperateData memory d_ = _testOperate(
                    vaults_[i],
                    vaultTypes_[j],
                    _convertOperateParamsToWei(vaults_[i], vaultTypes_[j], params_[j]),
                    alice,
                    0,
                    "test_liquidate(supply and borrow)"
                );
    ...
    function test_rebalance() public {
        ...
        RebalanceParams[4] memory params_ = [
            RebalanceParams({ depositOrWithdraw: 1, borrowOrPayback: 0, timeToSkip: 30 days }),
            RebalanceParams({ depositOrWithdraw: 0, borrowOrPayback: 1, timeToSkip: 30 days }),
            RebalanceParams({ depositOrWithdraw: 2, borrowOrPayback: 0, timeToSkip: 30 days }),
            RebalanceParams({ depositOrWithdraw: 0, borrowOrPayback: 2, timeToSkip: 30 days })
        ];
